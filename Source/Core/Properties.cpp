...
Properties::Properties(File* file, const char8* name, const char8* id, const char8* parentID, Properties* parent)    
{
	_namespace = name;
	_variables = NULL;
	_visited = false;
            ...
            else
            {
                ...
                if (value != NULL && value[0] == '{')
                {
                    ...
                    if (rccc && rccc == lineEnd)
                    {
                        ...
                        while (readChar(file) != '}')
                        {
                            if (file->Seek(-2, EFST_CURR) == false)
                            {
                                //GP_ERROR("Failed to Seek back to before a '}' character in properties file.");
                                return;
                            }
                    ...
                    Properties* space = new Properties(file, name, NULL, parentID, this);
                ...
                else
                {
                    ...
                    if (rc != NULL)
                    {
                        ...
                        Properties* space = new Properties(file, name, value, parentID, this);
                        _namespaces.push_back(space);
...
void Properties::mergeWith(Properties* overrides)
{
    ...
    overrides->rewind();
    //const char8* name = overrides->GetNextProperty();
    //while (name)
    //{
...
float32 Properties::GetFloat(const char8* name) const
{
    ...
    if (valueString)
    {
        ...
        if (scanned != 1)
        {
            //GP_ERROR("Error attempting to parse property '%s' as a float32.", name);
            return 0.0f;
        }
        return value;
    }
