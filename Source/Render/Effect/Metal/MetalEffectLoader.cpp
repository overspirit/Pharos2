...
MetalEffectLoader::MetalEffectLoader()
{
	m_funcList["include"] = (LOAD_FUNC)&MetalEffectLoader::LoadInclude;
	m_funcList["constant"] = (LOAD_FUNC)&MetalEffectLoader::LoadConstant;
	m_funcList["variable"] = (LOAD_FUNC)&MetalEffectLoader::LoadVariable;
	m_funcList["macro"] = (LOAD_FUNC)&MetalEffectLoader::LoadMacro;
	m_funcList["struct"] = (LOAD_FUNC)&MetalEffectLoader::LoadStruct;
	m_funcList["shader"] = (LOAD_FUNC)&MetalEffectLoader::LoadShader;
	m_funcList["state"] = (LOAD_FUNC)&MetalEffectLoader::LoadState;
	m_funcList["technique"] = (LOAD_FUNC)&MetalEffectLoader::LoadTechnique;

	m_enumList["cull_mode"] = (ENUM_FUNC)&MetalEffectLoader::GetEnumFromStr<ECullMode, CullModeDefine>;
}
...
bool MetalEffectLoader::Load(const char8* szPath)
{	
	...
	//////////////////////////////////////////////////////////////////////////
 	for (uint32 i = 0; i < (uint32)m_techniqueList.size(); i++)
 	{
		...
		MetalRenderTechInfo* techInfo = new MetalRenderTechInfo();
		...
		for (uint32 j = 0; j < (uint32)m_variableList.size(); j++)
		{
			...
			uint32 varBind = (uint32)strtol(m_variableList[j].bind.c_str() + 1, NULL, 10);
			...
			for (uint32 j = 0; j < (uint32)m_constantList.size(); j++)
		{
			...
			uint32 bind = (uint32)strtol(m_constantList[j].bind.c_str() + 1, NULL, 10);
...
bool MetalEffectLoader::LoadConstant(XmlNode* pNode)
{
...
bool MetalEffectLoader::LoadVariable(XmlNode* pNode)
{
...
bool MetalEffectLoader::LoadStruct(XmlNode* pNode)
{
...
bool MetalEffectLoader::LoadMacro(XmlNode* pNode)
{
...
bool MetalEffectLoader::LoadInclude(XmlNode* pNode)
{
...
bool MetalEffectLoader::LoadShader(XmlNode* pNode)
{
	for (uint32 i = 0; i < pNode->GetChildNum(); i++)
	{
...
bool MetalEffectLoader::LoadState(XmlNode* pNode)
{
	return true;
}

bool MetalEffectLoader::LoadTechnique(XmlNode* pNode)
{
