...
namespace Pharos
{
	namespace Render
	{
		...
		//例子如下
		//Metal_SUBRESOURCE_DATA data;
		//data.pSysMem = pData;
		//data.SysMemPitch = 4 * width;		//必须指定SysMemPitch才能建立成功
		//data.SysMemSlicePitch = 0;
		//D3D文档中有说明，SysMemPitch在建立Texture2D和Texture3D中使用，另外一个值SysMemSlicePitch
		//在建立Texture3D中使用，具体查看Metal_SUBRESOURCE_DATA的说明

		//2, Metal中的format中比D3D9要严格的多。D3D9中A8R8G8B8就是指4个分量都是
		//BYTE型，使用时直接就将分量值映射为0-1.0f的颜色值。而Metal中的R8G8B8A8则
		//有好几种类型，只有DXGI_FORMAT_R8G8B8A8_UNORM这个类型才会认为分量值
		//是BYTE类型，然后将分量值映射为0-1.0f的浮点颜色值。具体查看关于format中的说明

		//3, usage为Metal_USAGE_DYNAMIC，miplevels必须为1,不知道为什么。
		//而且这时只能map,且map的参数只能是Metal_MAP_WRITE_DISCARD,不能使用其他参数。
		//UpdateSubResource无效，D3D文档中说明UpdaeSubResource不能用于多重采样的文理，
		//不知道是不是这个原因。

		//4, UpdateSubResource这个函数可以用在使用Metal_USAGE_DEFAULT建立的mipLevels为1的纹理上
		// 其他mipLeves数值无效，这时不能map....

		//5, map的效率比UpdateSubResource略高一点，为了实现CopyFromData和CopyRect这两个函数
		//所以在Create纹理时我只使用了mipLevels为1,Usage为Metal_USAGE_DEFAULT建立。
		//主要考虑纯动态纹理这个东西使用的比较少，大部分动态的纹理都用于Render Target.

		//感觉Metal的纹理是个巨大的坑...

		class MetalSampleState;
